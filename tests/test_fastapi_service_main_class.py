...
@patch('fastapi_service.main.predictor')
def test_predict_item(predictor):
    prediction = np.ones(1, dtype=int)
    predictor.predict = Mock(return_value=prediction)

    transaction = Transaction.parse_obj(json.loads(initial_trn_json))
    actual = fastapi_service.main.predict_item(transaction)
    predictor.predict.assert_called_once_with(transaction)
    assert prediction[0] == actual


@patch('fastapi_service.main.predictor')
def test_predict_items(predictor):
    prediction = np.ones(1, dtype=int)
    predictor.predict = Mock(return_value=prediction)
    transaction_list = [Transaction.parse_obj(json.loads(initial_trn_json))]
    actual = fastapi_service.main.predict_items(transaction_list)
    predictor.predict.assert_called_once_with(transaction_list)
    assert prediction.tolist() == actual


async def get_trn():
    return Transaction.parse_obj(json.loads(initial_trn_json))

async def get_none():
    return None

@pytest.mark.asyncio
@patch('fastapi_service.main.db')
async def test_add_item(db_mock):
    transaction = Transaction.parse_obj(json.loads(initial_trn_json))

    db_mock.get = Mock(return_value=get_trn())
    actual = await fastapi_service.main.add_item(transaction)
    assert {'id': transaction.TransactionID, 'status': 'already in db'} == actual
    db_mock.get.assert_called_once_with(transaction.TransactionID)

    db_mock.get = Mock(return_value=get_none())
    db_mock.put = Mock(return_value=get_none())
    actual = await fastapi_service.main.add_item(transaction)
    assert {'id': transaction.TransactionID, 'status': 'added'} == actual
    db_mock.get.assert_called_once_with(transaction.TransactionID)
    db_mock.put.assert_called_once_with(transaction)


@patch('fastapi_service.main.predictor')
def test_train_model_on_input_data(predictor):
    predictor.train = Mock()
    dict_for_df = defaultdict(list)
    for k, v in json.loads(initial_trn_json).items():
        dict_for_df[k].append(v)
    fastapi_service.main.train_model_on_input_data(dict_for_df)
    predictor.train.assert_called_once_with(dict_for_df)


@pytest.mark.asyncio
@patch('fastapi_service.main.db')
@patch('fastapi_service.main.predictor')
async def test_train_model_on_full_data(predictor, db_mock):
    predictor.train = Mock()
    dict_for_df = defaultdict(list)
    for k, v in json.loads(initial_trn_json).items():
        dict_for_df[k].append(v)
    transaction_in_request = Transaction.parse_obj(json.loads(initial_trn_json))

    transaction_in_db = Transaction.parse_obj(json.loads(initial_trn_json))
    transaction_in_db.TransactionID = 1
    async def get_all():
        return [transaction_in_db]
    db_mock.get_all = Mock(return_value=get_all())
    db_mock.put = Mock(return_value=get_none())

    await fastapi_service.main.train_model_on_full_data(dict_for_df)
    db_mock.get_all.assert_called_once()
    db_mock.put.assert_called_once_with(transaction_in_request)
    predictor.train.assert_called_once_with([transaction_in_db, transaction_in_request])
